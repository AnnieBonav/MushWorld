        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Interpolators i) : SV_Target
            {
                ...
                // float4 noice = tex2D(_MainTex, i.uv);
                float4 noice = tex2D(_MainTex, float2(i.uv.x, (i.uv.y / _Speed/4))); // This starts changing over time, maybe already fixed
                float clamped = clamp(noice, 0.2, 0.4); // Light grawy but lookable between 0.3 and 0.3
                float lighterClamped = clamped + 0.6;
                float4 noiceColor = noice * _NoiceColor;
                // return noice;
                // return clamped + 0.5; // Noise but lighter
                return _ColorA * lighterClamped;
